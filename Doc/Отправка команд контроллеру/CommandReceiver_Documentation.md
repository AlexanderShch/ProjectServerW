...
| 0x04 | GET_CMD_INFO   | Request last received command audit (last command + device time + ack flag + status) | 6 bytes |

---

#### GET_CMD_INFO: response payload (Data, 6 bytes)

Purpose: return diagnostic info about the **last command received by the device** and whether the device considers it processed / acknowledged.

Data format (6 bytes):

```
Byte 0: lastCmdType            (uint8)  - last received command type (CmdType)
Byte 1: lastCmdCode            (uint8)  - last received command code
Byte 2-3: lastCmdDeviceTimeSec (uint16) - device time in seconds since boot (same base as telemetry MSGQUEUE_OBJ_t.Time)
Byte 4: ackSent                (uint8)  - 1 if the device sent a response/ack, 0 otherwise
Byte 5: lastCmdStatus          (uint8)  - status that the device used in its response (CmdStatus)
```

Server can compute an approximate wall-clock timestamp for command reception by mapping `lastCmdDeviceTimeSec` to telemetry `Time` and the wall-clock time of the latest telemetry packet.
...
**Поддерживаемые коды:** GET_VERSION, GET_CONFIG, GET_CMD_INFO
...
**Команда от сервера:**
```
Байты: 03 04 00 XX XX
       ?  ?  ?  ???? CRC16
       ?  ?  ?????? Длина данных (0)
       ?  ????????? Код команды (GET_CMD_INFO)
       ???????????? Тип команды (REQUEST)
```

**Ответ контроллера:**
```
Байты: 80 04 00 06 .. .. .. .. .. .. XX XX
       ?  ?  ?  ?  ?  ?  ???? CRC16
       ?  ?  ?  ?  ???????? Данные (6 байт: lastCmdType/Code/time/ack/status)
       ?  ?  ?  ???????????? Длина данных (6)
       ?  ?  ??????????????? Статус (OK)
       ?  ????????? Код команды (GET_CMD_INFO)
       ????????????????????? Тип (RESPONSE)
