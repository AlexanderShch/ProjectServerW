...
- Для успешных команд (status == OK):
  ...
  - Извлекает данные для команд REQUEST:
    ...
    * GET_CMD_INFO - отображает аудит последней принятой команды (6 байт)
  - Логирует результат
...
### Пример 2: Запрос аудита последней команды от контроллера
...
if (SendCommandAndWaitResponse(cmd, response)) {
    // Используем полученный аудит последней команды (6 байт)
}
```
