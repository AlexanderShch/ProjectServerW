...
#### Описание команды GET_CMD_INFO (аудит последней принятой команды) в ProjectServerW

- **Назначение**: получить диагностическую информацию о том, **какую команду устройство приняло последней**, и что устройство считает результатом её обработки (включая признак отправки ответа/ACK).
- **Где используется в UI**: кнопка **«КОМАНДА»** на вкладке **«Настройки»**.
- **Ожидаемый ответ**: `DataLen = 6`, поля:
  - `lastCmdType`, `lastCmdCode`
  - `lastCmdDeviceTimeSec` (секунды с момента старта устройства; та же база, что `MSGQUEUE_OBJ_t.Time` в телеметрии)
  - `ackSent` (0/1)
  - `lastCmdStatus` (код статуса, который устройство выставляло в ответе)
- **Как сервер вычисляет реальное время**: берёт `lastTelemetryTime` (время прихода последнего пакета) и `Time` из телеметрии, и смещает на разницу секунд между `lastCmdDeviceTimeSec` и последним `Time`.

### Пример 2: Запрос версии прошивки
